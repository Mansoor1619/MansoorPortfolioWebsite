...
const Hero: React.FC = () => {
  ...
  // Filter social icons for hero section
  const socialIcons = iconsData.filter(icon => 
        ...
        <div className="text-center">
          ...
          <div className="flex justify-center space-x-6 mb-8">
            {socialIcons.map((social) => {
              const socialUrl = personalData[social.id as keyof typeof personalData];
              if (!socialUrl) return null;
              
              return (
                <a
                  key={social.id}
                  href={`${social.url}${socialUrl}`}
                  target="_blank"
